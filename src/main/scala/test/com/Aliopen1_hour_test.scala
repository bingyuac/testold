...
/**
  ...
  */
object Aliopen1_hour_test {
  def main(args: Array[String]) {
    //val sparkConf = new SparkConf().setMaster("yarn-client").setAppName("LogStreaming_hour")spark://open003:7077
    //val sparkConf = new SparkConf().setMaster("local[4]").set("spark.sql.shuffle.partitions","20").setAppName("Aliopen1Streaming")
    val sparkConf = new SparkConf().setMaster("local[4]").setAppName("Aliopen1_hourStreaming")
    //每60秒一个批次
    val ssc = new StreamingContext(sparkConf, Seconds(3600))
    //从Kafka中读取数据，topic为ali，该topic包含两个分区
    val kafkaStream = KafkaUtils.createStream(
      ssc,
      "60.205.152.16:2181,60.205.152.20:2181", //Kafka集群使用的zookeeper
      "test-consumer-group-test1", //该消费者使用的group.id
    ...
    kafkaStream.foreachRDD(foreachFunc = (rdd: RDD[Array[String]], time: Time) => {
      ...
      //val logDataFrame = rdd.map(w => new Aliopen1Log(w(0).subsstat_data_hourtring(0, 10),w(2),w(6))).toDF()
      ...
      val stat_data_hour_sql = "select \"3\" as client_id,\"1003\" as project_id,ip,substring(day,1,10) as time," +
        ...
        "group by substring(day,1,10),ip,split(geo,\",\")[0]," +
        "case when instr(split(geo,\",\")[1],'省')>0 then split(split(geo,\",\")[1],'省')[0] " +
        "when instr(split(geo,\",\")[1],'市')>0 then split(split(geo,\",\")[1],'市')[0] " +
        "when instr(split(geo,\",\")[1],'新疆')>0 then substring(split(geo,\",\")[1],1,2) " +
        "when instr(split(geo,\",\")[1],'内蒙古')>0 then substring(split(geo,\",\")[1],1,3) " +
      ...
      val stat_pv_hour_sql = "select \"3\" as client_id,\"1003\" as project_id,substring(day,1,10) as time,request,count(*) as counts,\"1\" as type" +
        " from aliopen1log group by substring(day,1,10),request"
      val stat_pv_url_hour_sql = "select \"3\" as client_id,\"1003\" as project_id,request,substring(day,1,10) as time,count(*) as counts,split(geo,\",\")[0] as country,case when instr(split(geo,\",\")[1],'省')>0 then split(split(geo,\",\")[1],'省')[0] when instr(split(geo,\",\")[1],'市')>0 then split(split(geo,\",\")[1],'市')[0] when instr(split(geo,\",\")[1],'新疆')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'内蒙古')>0 then substring(split(geo,\",\")[1],1,3) when instr(split(geo,\",\")[1],'宁夏')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'广西')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'西藏')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'香港')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'澳门')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'台湾')>0 then substring(split(geo,\",\")[1],1,2) else split(geo,\",\")[1] end as province,case when instr(split(geo,\",\")[2],'市')>0 then split(split(geo,\",\")[2],'市')[0] else split(geo,\",\")[2] end as city" +
        " from aliopen1log group by substring(day,1,10),request,split(geo,\",\")[0],case when instr(split(geo,\",\")[1],'省')>0 then split(split(geo,\",\")[1],'省')[0] when instr(split(geo,\",\")[1],'市')>0 then split(split(geo,\",\")[1],'市')[0] when instr(split(geo,\",\")[1],'新疆')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'内蒙古')>0 then substring(split(geo,\",\")[1],1,3) when instr(split(geo,\",\")[1],'宁夏')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'广西')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'西藏')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'香港')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'澳门')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'台湾')>0 then substring(split(geo,\",\")[1],1,2) else split(geo,\",\")[1] end,case when instr(split(geo,\",\")[2],'市')>0 then split(split(geo,\",\")[2],'市')[0] else split(geo,\",\")[2] end"
      //val stat_referrer_hour_sql = ""
      //val stat_referrer_url_hour_sql = ""
      val stat_request_hour_sql = "select \"3\" as client_id,substring(day,1,10) as time,ip,method,count(*) as counts,\"1003\" as project_id " +
        "from aliopen1log group by substring(day,1,10),ip,method"
      val stat_status_hour_sql = "select \"3\" as client_id,\"1003\" as project_id,ip,substring(day,1,10) as time,status,count(*) as counts" +
        " from aliopen1log group by substring(day,1,10),ip,status"
      val stat_status_url_hour_sql = "select \"3\" as client_id,\"1003\" as project_id,request,substring(day,1,10) as time,status,count(*) as counts " +
      ...
      dfWriter_data.jdbc("jdbc:mysql://101.200.218.23:3306/demo", "stat_data_hour", prop)
      dfWriter_data_url.jdbc("jdbc:mysql://101.200.218.23:3306/demo", "stat_data_url_hour", prop)
      dfWriter_ip.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_ip_hour", prop)
      dfWriter_pv.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_pv_hour", prop)
      dfWriter_pv_url.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_pv_url_hour", prop)
      dfWriter_stat_request.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_request_hour", prop)
      dfWriter_stat_status.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_status_hour", prop)
      dfWriter_stat_status_url.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_status_url_hour", prop)
