...
/**
  ...
  */
object Aliopen1_daily_test {
  def main(args: Array[String]) {
    //val sparkConf = new SparkConf().setMaster("yarn-client").setAppName("LogStreaming_daily")spark://open003:7077
    //val sparkConf = new SparkConf().setMaster("local[4]").set("spark.sql.shuffle.partitions","20").setAppName("Aliopen1Streaming")
    val sparkConf = new SparkConf().setMaster("local[4]").setAppName("Aliopen1_dailyStreaming")
    //每60秒一个批次
    val ssc = new StreamingContext(sparkConf, Seconds(86400))
    //从Kafka中读取数据，topic为ali，该topic包含两个分区
    val kafkaStream = KafkaUtils.createStream(
      ssc,
      "60.205.152.16:2181,60.205.152.20:2181,60.205.152.57", //Kafka集群使用的zookeeper
      "test-consumer-group-daily-test", //该消费者使用的group.id
    ...
    kafkaStream.foreachRDD(foreachFunc = (rdd: RDD[Array[String]], time: Time) => {
      ...
      //val logDataFrame = rdd.map(w => new Aliopen1Log(w(0).subsstat_data_dailytring(0, 10),w(2),w(6))).toDF()
      ...
      val stat_data_daily_sql = "select \"3\" as client_id,\"1003\" as project_id,ip,substring(day,1,8) as time," +
        ...
        "then split(split(geo,\",\")[2],'市')[0] else split(geo,\",\")[2] end as city from aliopen1log " +
        "group by substring(day,1,8),ip,split(geo,\",\")[0]," +
        "case when instr(split(geo,\",\")[1],'省')>0 then split(split(geo,\",\")[1],'省')[0] " +
        "when instr(split(geo,\",\")[1],'市')>0 then split(split(geo,\",\")[1],'市')[0] " +
        "when instr(split(geo,\",\")[1],'新疆')>0 then substring(split(geo,\",\")[1],1,2) " +
        "when instr(split(geo,\",\")[1],'内蒙古')>0 then substring(split(geo,\",\")[1],1,3) " +
      ...
      /*      val stat_ip_geography_sql = "select \"1\" as client_id,ip as c_ip,split(geo,\",\")[0] as country," +
        ...
        "from aliopen1log"*/
      //val stat_new_ip_sql = ""
      //val stat_os_daily_sql = ""
      //val stat_os_url_daily_sql = ""
      //val stat_page_time_sql = ""
      val stat_pv_daily_sql = "select \"3\" as client_id,\"1003\" as project_id,substring(day,1,8) as time,request,count(*) as counts,\"1\" as type" +
        " from aliopen1log group by substring(day,1,8),request"
      val stat_pv_url_daily_sql = "select \"3\" as client_id,\"1003\" as project_id,request,substring(day,1,8) as time,count(*) as counts,split(geo,\",\")[0] as country,case when instr(split(geo,\",\")[1],'省')>0 then split(split(geo,\",\")[1],'省')[0] when instr(split(geo,\",\")[1],'市')>0 then split(split(geo,\",\")[1],'市')[0] when instr(split(geo,\",\")[1],'新疆')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'内蒙古')>0 then substring(split(geo,\",\")[1],1,3) when instr(split(geo,\",\")[1],'宁夏')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'广西')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'西藏')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'香港')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'澳门')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'台湾')>0 then substring(split(geo,\",\")[1],1,2) else split(geo,\",\")[1] end as province,case when instr(split(geo,\",\")[2],'市')>0 then split(split(geo,\",\")[2],'市')[0] else split(geo,\",\")[2] end as city" +
        " from aliopen1log group by substring(day,1,8),request,split(geo,\",\")[0],case when instr(split(geo,\",\")[1],'省')>0 then split(split(geo,\",\")[1],'省')[0] when instr(split(geo,\",\")[1],'市')>0 then split(split(geo,\",\")[1],'市')[0] when instr(split(geo,\",\")[1],'新疆')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'内蒙古')>0 then substring(split(geo,\",\")[1],1,3) when instr(split(geo,\",\")[1],'宁夏')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'广西')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'西藏')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'香港')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'澳门')>0 then substring(split(geo,\",\")[1],1,2) when instr(split(geo,\",\")[1],'台湾')>0 then substring(split(geo,\",\")[1],1,2) else split(geo,\",\")[1] end,case when instr(split(geo,\",\")[2],'市')>0 then split(split(geo,\",\")[2],'市')[0] else split(geo,\",\")[2] end"
      //val stat_referrer_daily_sql = ""
      //val stat_referrer_url_daily_sql = ""
      val stat_request_daily_sql = "select \"3\" as client_id,substring(day,1,8) as time,ip,method,count(*) as counts,\"1003\" as project_id " +
        "from aliopen1log group by substring(day,1,8),ip,method"
      val stat_status_daily_sql = "select \"3\" as client_id,\"1003\" as project_id,ip,substring(day,1,8) as time,status,count(*) as counts" +
        " from aliopen1log group by substring(day,1,8),ip,status"
      val stat_status_url_daily_sql = "select \"3\" as client_id,\"1003\" as project_id,request,substring(day,1,8) as time,status,count(*) as counts " +
        "from aliopen1log group by substring(day,1,8),request,status"
      //val stat_top_page_sql = ""
      //val stat_tp_daily_sql = ""
      ...
      val stat_data_daily = sqlContext.sql(stat_data_daily_sql)
      val stat_data_url_daily = sqlContext.sql(stat_data_url_daily_sql)
      val stat_ip_daily = sqlContext.sql(stat_ip_daily_sql)
      val stat_pv_daily = sqlContext.sql(stat_pv_daily_sql)
      val stat_pv_url_daily = sqlContext.sql(stat_pv_url_daily_sql)
      val stat_request_daily = sqlContext.sql(stat_request_daily_sql)
      val stat_status_daily = sqlContext.sql(stat_status_daily_sql)
      val stat_status_url_daily = sqlContext.sql(stat_status_url_daily_sql)
      ...
      stat_data_daily.show()
      stat_data_url_daily.show()
      stat_ip_daily.show()
      stat_pv_daily.show()
      stat_pv_url_daily.show()
      stat_request_daily.show()
      stat_status_daily.show()
      stat_status_url_daily.show()
      //val dfWriter_test = test.write.mode("append")
      val dfWriter_data = stat_data_daily.write.mode("append")
      //val dfWriter = stat_data_daily.write.mode("append")
      val dfWriter_data_url = stat_data_url_daily.write.mode("append")
      val dfWriter_ip = stat_ip_daily.write.mode("append")
      val dfWriter_pv = stat_pv_daily.write.mode("append")
      val dfWriter_pv_url = stat_pv_url_daily.write.mode("append")
      val dfWriter_stat_request = stat_request_daily.write.mode("append")
      val dfWriter_stat_status = stat_status_daily.write.mode("append")
      val dfWriter_stat_status_url = stat_status_url_daily.write.mode("append")
      ...
      dfWriter_data.jdbc("jdbc:mysql://101.200.218.23:3306/demo", "stat_data_daily", prop)
      dfWriter_data_url.jdbc("jdbc:mysql://101.200.218.23:3306/demo", "stat_data_url_daily", prop)
      dfWriter_ip.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_ip_daily", prop)
      dfWriter_pv.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_pv_daily", prop)
      dfWriter_pv_url.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_pv_url_daily", prop)
      dfWriter_stat_request.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_request_daily", prop)
      dfWriter_stat_status.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_status_daily", prop)
      dfWriter_stat_status_url.jdbc("jdbc:mysql://101.200.218.23:3306/demo","stat_status_url_daily", prop)
      //dfWriter_stat_ip_geography.jdbc("jdbc:mysql://101.200.218.23:3306/open03","stat_ip_geography", prop)
      //stat_data_daily.foreachPartition(myFun)
    })
    ssc.start()
  ...
  def myFun(iterator: Iterator[(String, Int)]): Unit = {
    ...
    val sql = "insert into stat_data_daily(client_id, project_id, c_ip, time, bytes) values (1, 1, ?, ?, ?)"
    try {
